...
object day02 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val strategyGuide: List[(Choice, Choice)] = strategyGuideRaw.map(pair => (convert(pair._1), convert(pair._2)))
    val strategyGuideSwapped = strategyGuide.map(pair => (pair._2, pair._1))
    ...
    val totalScore: Int = strategyGuideSwapped.map(pair => play(pair._1, pair._2)).sum
    println(totalScore)
  }
