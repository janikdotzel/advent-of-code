...
object day03 extends App {
  ...
  case class ItemList(items: String)

  case class Rucksack(itemList: ItemList) {
    val (compartment1, compartment2) = itemList.items.splitAt(itemList.items.length / 2)
  }

  def findDuplicateItemType(rucksack: Rucksack): Char = {
    var duplicate: Char = ' '
    for (item <- rucksack.compartment1) {
  ...
  val itemLists: List[ItemList] = input.split("\n").map(x => ItemList(x)).toList
  val rucksacks: List[Rucksack] = itemLists.map(x => Rucksack(x))
  val sum = rucksacks.map(r => convertToPriority(findDuplicateItemType(r))).sum
  println(sum)
}
